...
import numpy as np
# from ..General_Utilities.util import assert_nD
# from c_utils import _glcm_loop
from util import assert_nD
...
def glcm_features_calc(P, props=['contrast', 'dissimilarity', 'homogeneity', 'ASM','energy','correlation'], distances=None, angles=None, out='list'):
    ...
    for p_idx, current_prop in enumerate(props):
        ...
        if current_prop == 'energy':
            ...
            temp = np.apply_over_axes(np.sum, (P * weights), axes=(0, 1))[0, 0]
...
def feature_count(distances=[1,2], angles=[0, np.pi/4, np.pi/2, 3*np.pi/4], props=['contrast', 'dissimilarity', 'homogeneity', 'ASM','energy','correlation']):
    if isinstance(props, basestring):
        props = [props,]
    return len(distances) * len(angles) * len(props)

def featurename_strings(distances=[1,2], angles=[0, np.pi/4, np.pi/2, 3*np.pi/4], props=['contrast', 'dissimilarity', 'homogeneity', 'ASM','energy','correlation']):
...
if __name__ == "__main__":
    ...
    # print greycoprops(glcm_2d_aggregate(test_array, [1,2], [0, np.pi/4, np.pi/2, 3*np.pi/4], levels=5, symmetric=True, method='maximal_slice'), out='list', distances=[1,2], angles=[0, np.pi/4, np.pi/2, 3*np.pi/4])
    # print glcm_2d_aggregate(test_array, [1,2], [0, np.pi/4, np.pi/2, 3*np.pi/4], levels=5, symmetric=True, method='maximal_slice').shape