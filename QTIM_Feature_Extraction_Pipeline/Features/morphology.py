...
def calc_surface_area(image_numpy, pixdims, mask_value=0):
    ...
    # edge_image = ndimage.convolve(label_numpy, edges_kernel, mode='constant')
    edge_image = signal.convolve(label_numpy, edges_kernel,mode='same')
    # edge_image = convolve_3d(label_numpy, edges_kernel)
...
def morphology_features(image, pixdims, features=['volume','surface_area','volume_surface_area_ratio','compactness','compactness_alternate','spherical_disproportion','sphericity'], mask_value=0):
